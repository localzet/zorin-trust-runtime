...
case "$ABI" in arm64-v8a|armeabi-v7a|x86_64|x86) ;; *) echo "Unsupported ABI: $ABI" >&2; exit 2;; esac
BIN="$CORE_ROOT/bin/zorin-adb-core-$ABI"
[ -f "$BIN" ] || BIN="$CORE_ROOT/build/zorin-adb-core-$ABI"
[ -f "$BIN" ] || { echo "Bridge binary not found for $ABI" >&2; exit 3; }
if command -v openssl >/dev/null 2>&1; then TOKEN=$(openssl rand -hex 16); else TOKEN=$(od -An -N16 -tx1 /dev/urandom | tr -d ' \n'); fi
...
adb shell am start -n dev.zorin.nativelab/android.app.NativeActivity --es dev.zorin.bridge.token "$TOKEN"
