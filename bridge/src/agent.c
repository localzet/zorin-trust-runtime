...
// Аутентифицированный read-only localhost backend возможностей для Zorin Trust Runtime.
// Произвольного выполнения команд нет: каждая наружная операция — фиксированная диагностика.
...
static int token_equal(const char* a, const char* b) {
    // Сравниваем за постоянное число шагов для фиксированной максимальной длины токена. Канал локальный,
    // но отказ от early-out почти ничего не стоит и держит границу аккуратной.
    unsigned int diff=0;
    int enda=0, endb=0;
    for(int i=0; i<95; ++i) {
...
static int parse_port(const char* s) {
    int v=0;
    if(!s||!*s) return DEFAULT_PORT;
    while(*s>='0'&&*s<='9') {
        v=v*10+(*s-'0');
        ++s;
        if(v>65535)return DEFAULT_PORT;
    }
...
static int status_value(const char* key, char* out, int cap) {
    char b[8192];
    if(read_small("/proc/self/status", b, sizeof(b))<0)return -1;
    char* p=strstr(b, key);
    if(!p)return -1;
    p+=strlen(key);
    while(*p==' '||*p=='\t')++p;
...
static void get_context(char* out, int cap) {
    ...
    if(rc<0) snprintf(out,(size_t)cap, "ERRNO %d", -rc);
}
...
static void command_hello(int fd) {
    // HELLO намеренно не раскрывает privileged-значения, а только объявляет protocol/auth mode.
...
static void command_status(int fd) {
    ...
    if(erc<0)snprintf(enforce, sizeof(enforce), "ERRNO %d", -erc);
    char t[192];
...
static void command_services(int fd) {
    ...
    for(int i=0; i<total; ++i) {
        ...
        if(pos>(int)sizeof(out)-256)break;
    }
...
static void command_kernel(int fd) {
    ...
    else append_kv(out, sizeof(out), &pos, "kernel", "unavailable");
    ...
    while(*p) {
        ...
        if(line>2) {
            ...
            if(c<e) {
                ...
                while(z>s&&(z[-1]==' '||z[-1]=='\t'))--z;
                if(z>s) {
                    if(pos&&pos<cap-1)out[pos++]=',';
                    while(s<z&&pos<cap-1)out[pos++]=*s++;
                }
            }
        }
        p=*e?e+1:e;
...
static void read_sysctl_kv(char* out, int cap, int* pos, const char* key, const char* path) {
    ...
    if(rc>=0)append_kv(out, cap, pos, key, v);
    else {
...
static void command_security(int fd) {
    ...
    if(rc>=0)append_kv(out, sizeof(out), &pos, "selinux_enforce", v);
    else {
        snprintf(v, sizeof(v), "ERRNO %d", -rc);
        append_kv(out, sizeof(out), &pos, "selinux_enforce", v);
    }
    const char* props[]= {
        "ro.boot.verifiedbootstate", "ro.boot.vbmeta.device_state", "ro.boot.flash.locked", "ro.debuggable", "ro.secure"
    };
    const char* keys[]= {
        "verified_boot", "vbmeta_state", "flash_locked", "debuggable", "ro_secure"
    };
    for(int i=0; i<5; ++i) {
        if(prop_get(props[i], v, sizeof(v)))append_kv(out, sizeof(out), &pos, keys[i], v);
        else append_kv(out, sizeof(out), &pos, keys[i], "not_exposed");
    }
...
static void command_process(int fd) {
    ...
    const char* fields[]= {
        "Threads:", "VmRSS:", "VmSize:", "TracerPid:", "Seccomp:", "NoNewPrivs:", "CapEff:"
    };
    const char* keys[]= {
        "threads", "rss", "vm_size", "tracer_pid", "seccomp", "no_new_privs", "cap_eff"
    };
...
static void command_probes(int fd) {
    const char* keys[]= {
        "io_uring_setup", "bpf_map_create", "perf_event_open", "userfaultfd", "pidfd_open", "memfd_create", "membarrier_query",
        "landlock_abi", "setns_self_net"
    };
    char out[3072], v[128];
    int pos=0;
    for(int i=0; i<9; ++i) {
...
static void path_visibility(char* out, int cap, int* pos, const char* key, const char* path) {
    char v[256];
    int rc=read_small(path, v, sizeof(v));
    if(rc>=0)append_kv(out, cap, pos, key, "VISIBLE");
    else {
        snprintf(v, sizeof(v), "ERRNO %d", -rc);
        append_kv(out, cap, pos, key, v);
    }
...
static void command_procfs(int fd) {
    ...
    if(readlink_small("/proc/1/ns/mnt", v, sizeof(v))>=0)append_kv(out, sizeof(out), &pos, "pid1_ns_mnt", v);
    else append_kv(out, sizeof(out), &pos, "pid1_ns_mnt", "DENIED");
    if(readlink_small("/proc/1/ns/pid", v, sizeof(v))>=0)append_kv(out, sizeof(out), &pos, "pid1_ns_pid", v);
    else append_kv(out, sizeof(out), &pos, "pid1_ns_pid", "DENIED");
    if(readlink_small("/proc/1/ns/net", v, sizeof(v))>=0)append_kv(out, sizeof(out), &pos, "pid1_ns_net", v);
    else append_kv(out, sizeof(out), &pos, "pid1_ns_net", "DENIED");
    send_text(fd, out);
}
...
int main(int argc, char** argv) {
    ...
    for(int i=1; i<argc; ++i) {
        if(streq(argv[i], "--allow-root")) allow_root=1;
            ...
            // После старта сокращаем время жизни токена в /proc/<pid>/cmdline.
    ...
    getuid(), getpid(), g_require_auth?"required":"disabled");
