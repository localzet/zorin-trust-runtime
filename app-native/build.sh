...
python3 "$HERE/tools/verify_process_ipc.py" "$HERE"
python3 "$HERE/tools/verify_multi_host.py"
python3 "$HERE/tools/verify_portable_transport.py"
...
python3 "$HERE/tools/apk_v2.py" sign \
  ...
  "$BUILD/zorin-trust-runtime-v8.2.0.apk" \
  --key "$KEY" --cert "$CERT"

# Если установлены официальные Android Build Tools, дополнительно проверяем APK через apksigner.
if command -v apksigner >/dev/null 2>&1; then
  apksigner verify --verbose --print-certs "$BUILD/zorin-trust-runtime-v8.2.0.apk"
fi

python3 "$HERE/tools/apk_v2.py" verify "$BUILD/zorin-trust-runtime-v8.2.0.apk"
sha256sum "$BUILD/zorin-trust-runtime-v8.2.0.apk"
