...
def rlen(buf, p):
    ...
    if b < 128:
        return (b, p + 1)
    return ((b & 127) << 8 | buf[p + 1], p + 2)
while pos < len(d):
    t, hs, sz = struct.unpack_from('<HHI', d, pos)
    if t == 1:
        sc, _, flags, ss, _ = struct.unpack_from('<IIIII', d, pos + 8)
        offs = struct.unpack_from('<' + 'I' * sc, d, pos + hs)
        base = pos + ss
        for o in offs:
            q = base + o
            _, q = rlen(d, q)
            bl, q = rlen(d, q)
            strings.append(d[q:q + bl].decode())
    elif t == 384:
        n = (sz - 8) // 4
        resmap = list(struct.unpack_from('<' + 'I' * n, d, pos + 8))
    elif t == 258:
        ...
        for i in range(acount):
            ...
            nm = strings[ni]
            rid = resmap[ni] if ni < len(resmap) else None
...
if a.get('process', (None, None, None))[1] != ':trust':
    raise SystemExit(f"FAIL: service process={a.get('process')}")
if a.get('foregroundServiceType', (None, None, None))[1] != 1073741824:
    raise SystemExit('FAIL: specialUse FGS type missing')
rids = [rid for _, rid, _, _ in service if rid is not None]
if rids != sorted(rids):
    raise SystemExit(f'FAIL: service attrs not sorted by resource id: {[hex(x) for x in rids]}')
