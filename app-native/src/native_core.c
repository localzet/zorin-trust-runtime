...
// Публичный NDK Binder API (libbinder_ndk.so, API 29+).
...
extern void AIBinder_decStrong(AIBinder*);
    extern AIBinder_Class* AIBinder_Class_define(const char*, void*(*)(void*), void(*)(void*), int(*)(AIBinder*, uint32_t,
...
// lifetime процесса trust/runtime
static volatile int g_ui_alive = 0;
// lifetime экземпляра NativeActivity
static volatile unsigned int g_ui_generation = 0;
static volatile int g_selected_tab = 2;
static volatile int g_developer_mode = 0;
static volatile int g_product_page = 0;
/* 0 Home, 1 Requests, 2 Devices, 3 Settings */
static int g_product_nav[4][4];
static int g_authority_allow_rect[4];
static int g_authority_deny_rect[4];
static int g_safe_left=0, g_safe_top=0, g_safe_right=0, g_safe_bottom=0;
...
static void outline_rect(ANativeWindow_Buffer* b, int x, int y, int w, int h, int t, uint32_t c) {
    fill_rect(b, x, y, w, t, c);
    fill_rect(b, x, y+h-t, w, t, c);
    fill_rect(b, x, y, t, h, c);
    fill_rect(b, x+w-t, y, t, h, c);
}
static void fill_circle(ANativeWindow_Buffer* b, int cx, int cy, int r, uint32_t color) {
    ...
    int y0=cy-r, y1=cy+r;
    if(y0<0)y0=0;
    if(y1>=b->height)y1=b->height-1;
    int x0=cx-r, x1=cx+r;
    if(x0<0)x0=0;
    if(x1>=b->width)x1=b->width-1;
    for(int y=y0; y<=y1; ++y) {
        int dy=y-cy;
        for(int x=x0; x<=x1; ++x) {
            int dx=x-cx;
            if(dx*dx+dy*dy<=r2)p[y*b->stride+x]=color;
        }
    }
}
static void draw_ring(ANativeWindow_Buffer* b, int cx, int cy, int r, int thickness, uint32_t color, uint32_t inner) {
    if(thickness<1)thickness=1;
    fill_circle(b, cx, cy, r, color);
    fill_circle(b, cx, cy, r-thickness, inner);
}
...
static int init_runtime_context_from(JNIEnv* env, jobject obj) {
    ...
    if(g_app_context && !g_files_dir[0]) {
        ...
        if(ps) {
            ...
            if(c) {
                snprintf(g_files_dir, sizeof(g_files_dir), "%s", c);
                (*env)->ReleaseStringUTFChars(env, ps, c);
            }
        }
        if((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionClear(env);
...
static int copy_text_to_clipboard(const char* text) {
    ...
    jstring service_name =(*env)->NewStringUTF(env, "clipboard");
        jobject clipboard =(get_service && service_name) ?(*env)->CallObjectMethod(env, g_activity->clazz, get_service,
    service_name) : 0;
    jclass clipdata_cls =(*env)->FindClass(env, "android/content/ClipData");
    jmethodID new_plain = clipdata_cls
    ...
    jstring body =(*env)->NewStringUTF(env, text);
        jobject clip =(new_plain && label && body) ?(*env)->CallStaticObjectMethod(env, clipdata_cls, new_plain, label,
    body) : 0;
...
// ---- lifecycle TrustService + Trust Visual Channel ---------------------------------
// TrustService намеренно оставлен крошечным DEX shim. Всё ниже остаётся native.
...
static int trust_request_overlay_permission(void) {
    ...
    if(in && uri && setData) {
        jobject tmp =(*env)->CallObjectMethod(env, in, setData, uri);
        if(tmp)(*env)->DeleteLocalRef(env, tmp);
    }
    if(in && addFlags) {
        jobject tmp =(*env)->CallObjectMethod(env, in, addFlags, 0x10000000);
        if(tmp)(*env)->DeleteLocalRef(env, tmp);
    }
    jclass cc =(*env)->GetObjectClass(env, ctx);
    jmethodID start = cc ?(*env)->GetMethodID(env, cc, "startActivity", "(Landroid/content/Intent;)V") : 0;
    if(start && in) {
        (*env)->CallVoidMethod(env, ctx, start, in);
        rc = 0;
    }
    if((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
...
static int trust_start_service_from_context(jobject ctx, int pulse) {
    ...
    if(in && scn && pkg && cls) {
        jobject t =(*env)->CallObjectMethod(env, in, scn, pkg, cls);
        if(t)(*env)->DeleteLocalRef(env, t);
    }
    jstring ensurek =(*env)->NewStringUTF(env, "dev.zorin.trust.ensure");
    if(in && pe && ensurek) {
        jobject t =(*env)->CallObjectMethod(env, in, pe, ensurek, JNI_TRUE);
        if(t)(*env)->DeleteLocalRef(env, t);
    }
    if(pulse) {
        jstring pulsek =(*env)->NewStringUTF(env, "dev.zorin.trust.pulse");
        if(in && pe && pulsek) {
            jobject t =(*env)->CallObjectMethod(env, in, pe, pulsek, JNI_TRUE);
            if(t)(*env)->DeleteLocalRef(env, t);
        }
...
static int trust_start_foreground_notification(JNIEnv* env, jobject service) {
    ...
    jmethodID ncctor = ncc ?(*env)->GetMethodID(env, ncc, "<init>", "(Ljava/lang/String;Ljava/lang/CharSequence;I)V") : 0;
        jobject channel =(ncctor && channel_id && channel_name) ?(*env)->NewObject(env, ncc, ncctor, channel_id, channel_name,
    2) : 0;
    ...
    // Используем стабильный public framework drawable напрямую. Предыдущая реализация
    // искала OEM/system resource по имени; на некоторых HyperOS это возвращает 0,
    // из-за чего foreground notification становится невалидным и Android убивает
    // сервис вскоре после старта. android.R.drawable.ic_lock_lock = 0x0108002f.
    int icon = 0x0108002f;
    jclass rccls = 0;
    jobject res = 0;
    jstring iname = 0;
    ...
    if(b && ssi && icon) {
        jobject t=(*env)->CallObjectMethod(env, b, ssi, icon);
        if(t)(*env)->DeleteLocalRef(env, t);
    }
    if(b && sct && title) {
        jobject t=(*env)->CallObjectMethod(env, b, sct, title);
        if(t)(*env)->DeleteLocalRef(env, t);
    }
    if(b && scx && text) {
        jobject t=(*env)->CallObjectMethod(env, b, scx, text);
        if(t)(*env)->DeleteLocalRef(env, t);
    }
    if(b && song) {
        jobject t=(*env)->CallObjectMethod(env, b, song, JNI_TRUE);
        if(t)(*env)->DeleteLocalRef(env, t);
    }
    ...
    if(sf && notif) {
        (*env)->CallVoidMethod(env, service, sf, 7301, notif);
        rc=0;
    }
    if((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
    ...
    if(notif)(*env)->DeleteLocalRef(env, notif);
    ...
    if(apkg)(*env)->DeleteLocalRef(env, apkg);
    if(dtype)(*env)->DeleteLocalRef(env, dtype);
    if(iname)(*env)->DeleteLocalRef(env, iname);
    if(res)(*env)->DeleteLocalRef(env, res);
    if(rccls)(*env)->DeleteLocalRef(env, rccls);
    if(nmc)(*env)->DeleteLocalRef(env, nmc);
    if(channel)(*env)->DeleteLocalRef(env, channel);
    if(ncc)(*env)->DeleteLocalRef(env, ncc);
    if(channel_name)(*env)->DeleteLocalRef(env, channel_name);
    if(channel_id)(*env)->DeleteLocalRef(env, channel_id);
    if(nm)(*env)->DeleteLocalRef(env, nm);
    if(ns)(*env)->DeleteLocalRef(env, ns);
    if(sc)(*env)->DeleteLocalRef(env, sc);
    return rc;
}
static int trust_visual_show(JNIEnv* env, jobject service) {
    ...
    // Один прозрачный overlay view переиспользуем весь lifetime процесса; каждый импульс анимируем
    // от компактного ярко-красного кольца к большому прозрачному.
    ...
    if(!view) {
        ...
        float density=1.0f;
        jclass cc=(*env)->GetObjectClass(env, service);
        jmethodID gr=cc?(*env)->GetMethodID(env, cc, "getResources", "()Landroid/content/res/Resources;"):0;
        jobject r=gr?(*env)->CallObjectMethod(env, service, gr):0;
        jclass rc=r?(*env)->GetObjectClass(env, r):0;
        jmethodID gdm=rc?(*env)->GetMethodID(env, rc, "getDisplayMetrics", "()Landroid/util/DisplayMetrics;"):0;
        jobject dm=gdm?(*env)->CallObjectMethod(env, r, gdm):0;
        jclass dmc=dm?(*env)->GetObjectClass(env, dm):0;
        jfieldID df=dmc?(*env)->GetFieldID(env, dmc, "density", "F"):0;
        if(dm&&df)density=(*env)->GetFloatField(env, dm, df);
        int px=(int)(236.0f*density);
        if(px<180)px=180;
        int stroke=(int)(7.0f*density);
        ...
        jobject lp = lpctor ?(*env)->NewObject(env, lpc, lpctor, px, px, 2038, 0x118, -3) : 0;
        // OVERLAY, NOT_FOCUSABLE|NOT_TOUCHABLE|LAYOUT_IN_SCREEN, TRANSLUCENT
        ...
        if(!(*env)->ExceptionCheck(env) && wm && tv) {
            g_visual_wm=(*env)->NewGlobalRef(env, wm);
            g_visual_view=(*env)->NewGlobalRef(env, tv);
            view=g_visual_view;
        }
        if((*env)->ExceptionCheck(env))(*env)->ExceptionClear(env);
        if(wmc)(*env)->DeleteLocalRef(env, wmc);
        if(lp)(*env)->DeleteLocalRef(env, lp);
        if(lpc)(*env)->DeleteLocalRef(env, lpc);
        if(gd)(*env)->DeleteLocalRef(env, gd);
        if(gc)(*env)->DeleteLocalRef(env, gc);
        if(dmc)(*env)->DeleteLocalRef(env, dmc);
        if(dm)(*env)->DeleteLocalRef(env, dm);
        if(rc)(*env)->DeleteLocalRef(env, rc);
        if(r)(*env)->DeleteLocalRef(env, r);
        if(cc)(*env)->DeleteLocalRef(env, cc);
        if(txt)(*env)->DeleteLocalRef(env, txt);
        if(tv)(*env)->DeleteLocalRef(env, tv);
        if(tc)(*env)->DeleteLocalRef(env, tc);
        if(wm)(*env)->DeleteLocalRef(env, wm);
        if(wn)(*env)->DeleteLocalRef(env, wn);
        if(sc)(*env)->DeleteLocalRef(env, sc);
    }
    if(!view) return -3;
    ...
    if(sa)(*env)->CallVoidMethod(env, view, sa,(jfloat)1.0f);
    if(ssx)(*env)->CallVoidMethod(env, view, ssx,(jfloat)0.72f);
    if(ssy)(*env)->CallVoidMethod(env, view, ssy,(jfloat)0.72f);
    jobject a=anim?(*env)->CallObjectMethod(env, view, anim):0;
    jclass ac=a?(*env)->GetObjectClass(env, a):0;
    jmethodID cancel=ac?(*env)->GetMethodID(env, ac, "cancel", "()V"):0;
    if(a&&cancel)(*env)->CallVoidMethod(env, a, cancel);
    // После cancel берём свежий animator и настраиваем expand+fade.
    if(a)(*env)->DeleteLocalRef(env, a);
    if(ac)(*env)->DeleteLocalRef(env, ac);
    a=anim?(*env)->CallObjectMethod(env, view, anim):0;
    ac=a?(*env)->GetObjectClass(env, a):0;
    ...
    if(a&&aa) {
        jobject q=(*env)->CallObjectMethod(env, a, aa,(jfloat)0.0f);
        if(q)(*env)->DeleteLocalRef(env, q);
    }
    if(a&&ax) {
        jobject q=(*env)->CallObjectMethod(env, a, ax,(jfloat)1.18f);
        if(q)(*env)->DeleteLocalRef(env, q);
    }
    if(a&&ay) {
        jobject q=(*env)->CallObjectMethod(env, a, ay,(jfloat)1.18f);
        if(q)(*env)->DeleteLocalRef(env, q);
    }
    if(a&&delay) {
        jobject q=(*env)->CallObjectMethod(env, a, delay,(jlong)550);
        if(q)(*env)->DeleteLocalRef(env, q);
    }
    if(a&&dur) {
        jobject q=(*env)->CallObjectMethod(env, a, dur,(jlong)950);
        if(q)(*env)->DeleteLocalRef(env, q);
    }
    if(a&&start)(*env)->CallVoidMethod(env, a, start);
    int ok=1;
    if((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        ok=0;
    ...
    if(ac)(*env)->DeleteLocalRef(env, ac);
    if(a)(*env)->DeleteLocalRef(env, a);
    if(vc)(*env)->DeleteLocalRef(env, vc);
    return ok?0:-4;
}
static void trust_visual_remove(JNIEnv* env) {
    ...
    (*env)->DeleteGlobalRef(env, g_visual_view);
    g_visual_view=0;
    (*env)->DeleteGlobalRef(env, g_visual_wm);
    g_visual_wm=0;
}
...
JNIEXPORT void JNICALL Java_dev_zorin_trustruntime_TrustService_nativeOnCreate(JNIEnv* env, jclass cls, jobject service) {
    ...
    JavaVM* vm=0;
    if((*env)->GetJavaVM(env, &vm)==JNI_OK&&vm)g_vm=vm;
    (void)init_runtime_context_from(env, service);
    g_running=1;
    g_trust_service_alive=1;
    (void)trust_start_foreground_notification(env, service);
    trust_ui_publish_state();
    trust_start_worker();
}
__attribute__((visibility("default")))
    JNIEXPORT void JNICALL Java_dev_zorin_trustruntime_TrustService_nativeOnStart(JNIEnv* env, jclass cls, jobject service,
jobject intent) {
    (void)cls;
    if(!env||!service)return;
    g_running=1;
...
JNIEXPORT void JNICALL Java_dev_zorin_trustruntime_TrustService_nativeOnDestroy(JNIEnv* env, jclass cls, jobject service) {
    ...
    g_trust_service_alive=0;
    trust_ui_publish_state();
    g_running=0;
}
static int in_rect(int x, int y, const int r[4]) {
...
static void kv(ANativeWindow_Buffer* b, int* y, int x, const char* label, const char* value, int state, int scale) {
    ...
    if(!b) return;
    uint32_t fg = rgb(216, 226, 235), good = rgb(86, 232, 157), warn = rgb(255, 190, 82), bad = rgb(255, 105, 120),
    dim=rgb(129, 148, 164);
    ...
    int maxchars=(b->width-x-18)/(6*scale);
    if(maxchars<8)maxchars=8;
    int rows=((int)strlen(line)+maxchars-1)/maxchars;
    if(rows<1)rows=1;
...
static void section_note(ANativeWindow_Buffer* b, int* y, int x, const char* s, int scale) {
    ...
    int maxchars=(b->width-x-18)/(6*scale);
    if(maxchars<8)maxchars=8;
    int rows=((int)strlen(s)+maxchars-1)/maxchars;
    if(rows<1)rows=1;
    *y += rows*9*scale + 3*scale;
}
static void fmt_int(char* out, int cap, long v) {
    snprintf(out,(size_t)cap, "%ld", v);
}
    static void trust_draw_action(ANativeWindow_Buffer* b, int x, int y, int w, int h, const char* label, int rect[4],
int scale, int danger) {
    if(!b)return;
    uint32_t accent=danger?rgb(255, 105, 120):rgb(238, 72, 88), panel=danger?rgb(40, 20, 27):rgb(37, 17, 23);
    fill_rect(b, x, y, w, h, panel);
    outline_rect(b, x, y, w, h, 2, accent);
    int tw=text_width(label, scale);
    draw_text(b, x+(w-tw)/2, y+(h-7*scale)/2, label, scale, accent);
    rect[0]=x;
    rect[1]=y;
    rect[2]=w;
    rect[3]=h;
...
static void trust_pair_code(const char* fp, char* out, int cap) {
    static const char* words[16]= {
        "EMBER", "FALCON", "NOVA", "WOLF", "ORBIT", "ONYX", "PIXEL", "RAVEN", "SOLAR", "TITAN", "VECTOR", "COMET", "PULSE",
        "ATLAS", "NEXUS", "VAULT"
    };
    int n[8], k=0;
    for(int i=0; fp&&fp[i]&&k<8; ++i) {
        int v=trust_hexval(fp[i]);
        if(v>=0)n[k++]=v;
    }
    if(k<8) {
...
static void render_system(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    ...
    snprintf(v, sizeof(v), "SDK %d / RUN #%u",(int)g_activity->sdkVersion, g_run_counter);
    kv(b, y, x, "RUNTIME", v, 1, scale);
    struct utsname u;
    if(uname(&u)==0) {
        snprintf(v, sizeof(v), "%s %s", u.sysname, u.release);
        kv(b, y, x, "KERNEL", v, 1, scale);
        kv(b, y, x, "MACHINE", u.machine, 1, scale);
    }
    snprintf(v, sizeof(v), "%d BYTES", getpagesize());
    kv(b, y, x, "PAGE SIZE", v, 1, scale);
    ...
    if(prop_get("ro.build.type", v, sizeof(v))) kv(b, y, x, "BUILD TYPE", v, mini_streq(v, "user")?1:0, scale);
        kv(b, y, x, "SELINUX", selinux_mode(se, sizeof(se)), mini_streq(selinux_mode(se, sizeof(se)), "ENFORCING")?1:0,
    scale);
    if(read_small("/proc/self/attr/current", v, sizeof(v))>0) {
        trim_line(v);
        kv(b, y, x, "SECONTEXT", v, 1, scale);
    }
    if(read_small("/proc/self/cgroup", v, sizeof(v))>0) {
        trim_line(v);
        char* nl=strstr(v, "\n");
        if(nl)*nl=0;
        kv(b, y, x, "CGROUP", v, 1, scale);
    }
    ...
    if(readlink_small("/proc/self/ns/net", v, sizeof(v))>=0) kv(b, y, x, "NS NET", v, 1, scale);
        snprintf(v, sizeof(v), "B:%s H:%s V:%s", access("/dev/binder", F_OK)==0?"YES":"NO", access("/dev/hwbinder", F_OK)==0?"YES":"NO",
    access("/dev/vndbinder", F_OK)==0?"YES":"NO");
...
static void render_process(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    char v[256];
    snprintf(v, sizeof(v), "PID %d / TID %ld", getpid(), syscall(GETTID_NR));
    kv(b, y, x, "IDENTITY", v, 1, scale);
    snprintf(v, sizeof(v), "UID %u / GID %u", getuid(), getgid());
    kv(b, y, x, "CREDENTIALS", v, getuid()==0?0:1, scale);
    ...
    if(extract_status("NoNewPrivs:", v, sizeof(v))>=0) kv(b, y, x, "NO NEW PRIV", v, mini_streq(v, "1")?1:0, scale);
        if(extract_status("CapEff:", v, sizeof(v))>=0) kv(b, y, x, "CAP EFF", v, mini_streq(v, "0000000000000000")?1:0,
    scale);
    snprintf(v, sizeof(v), "%d (SCAN 0..511)", approx_fd_count());
    kv(b, y, x, "OPEN FDS", v, 1, scale);
    int maps=count_file_lines("/proc/self/maps");
    snprintf(v, sizeof(v), "%d", maps);
    kv(b, y, x, "MEM MAPS", v, maps>0?1:-1, scale);
...
static uint16_t net_u16(uint16_t v) {
    return(uint16_t)((v << 8) |(v >> 8));
}
static int bridge_token_from_intent(void) {
    g_bridge_token[0] = 0;
    g_bridge_token_saved = 0;
    ...
    if(value) {
        ...
        if(utf) {
            int i=0;
            while(utf[i]&&i<(int)sizeof(g_bridge_token)-1) {
                g_bridge_token[i]=utf[i];
                ++i;
            }
            g_bridge_token[i]=0;
    ...
    if(strlen(g_bridge_token)<16) {
        // В Intent нет пригодного токена: восстанавливаем последний токен с успешной аутентификацией.
        ...
        jstring prefs_name =(*env)->NewStringUTF(env, "zorin_native_lab");
            jobject prefs =(get_prefs && prefs_name) ?(*env)->CallObjectMethod(env, g_activity->clazz, get_prefs, prefs_name,
        0) : 0;
        ...
        if(saved) {
            const char* utf=(*env)->GetStringUTFChars(env, saved, 0);
            if(utf) {
                int i=0;
                while(utf[i]&&i<(int)sizeof(g_bridge_token)-1) {
                    g_bridge_token[i]=utf[i];
                ...
                g_bridge_token[i]=0;
                (*env)->ReleaseStringUTFChars(env, saved, utf);
                g_bridge_token_saved=(i>=16);
            }
            (*env)->DeleteLocalRef(env, saved);
        }
        if(pref_key)(*env)->DeleteLocalRef(env, pref_key);
        if(prefs_cls)(*env)->DeleteLocalRef(env, prefs_cls);
        if(prefs)(*env)->DeleteLocalRef(env, prefs);
        if(prefs_name)(*env)->DeleteLocalRef(env, prefs_name);
    }
    ...
    if(extra_key)(*env)->DeleteLocalRef(env, extra_key);
...
static int bridge_save_authenticated_token(void) {
    ...
    if((*vm)->GetEnv(vm,(void**)&env, JNI_VERSION_1_6)!=JNI_OK) {
        if((*vm)->AttachCurrentThread(vm,(void**)&env, 0)!=JNI_OK||!env)return -3;
        attached=1;
    }
    int rc=0;
    jclass activity_cls=(*env)->GetObjectClass(env, g_activity->clazz);
    ...
    jstring key=(*env)->NewStringUTF(env, "bridge_token");
    jstring val=(*env)->NewStringUTF(env, g_bridge_token);
    if(put&&key&&val&&editor) {
        jobject ignored=(*env)->CallObjectMethod(env, editor, put, key, val);
        if(ignored)(*env)->DeleteLocalRef(env, ignored);
        if(apply)(*env)->CallVoidMethod(env, editor, apply);
        g_bridge_token_saved=1;
    }
    else rc=-1;
    if((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
    ...
    if(editor_cls)(*env)->DeleteLocalRef(env, editor_cls);
    if(editor)(*env)->DeleteLocalRef(env, editor);
    if(prefs_cls)(*env)->DeleteLocalRef(env, prefs_cls);
    if(prefs)(*env)->DeleteLocalRef(env, prefs);
    if(prefs_name)(*env)->DeleteLocalRef(env, prefs_name);
    if(activity_cls)(*env)->DeleteLocalRef(env, activity_cls);
...
static int bridge_request(const char* command, char* out, int cap) {
    ...
    if(send(fd, wire, ncmd, 0) !=(ssize_t)ncmd) {
        int e=last_errno();
        close(fd);
        return e?e:-3;
    }
...
static void* core_class_create(void* args) {
    return args;
}
static void core_class_destroy(void* user) {
    (void)user;
}
...
static int core_read_status(AParcel* reply, char* err, int errcap) {
    ...
    if(rc != 0) {
        if(err)snprintf(err,(size_t)errcap, "STATUS_HEADER %d", rc);
        return rc;
    }
    if(!status) {
        if(err)snprintf(err,(size_t)errcap, "NO STATUS");
        return -1002;
    }
...
static int core_call_string(AIBinder* core, uint32_t code, char* out, int cap) {
    ...
    AParcel* reply=0;
    char err[128];
    int rc=core_prepare(core, &in);
    if(rc!=0) {
        snprintf(out,(size_t)cap, "PREP %d", rc);
        return rc;
    }
    rc=AIBinder_transact(core, code, &in, &reply, 0);
    if(rc!=0) {
        if(reply)AParcel_delete(reply);
        snprintf(out,(size_t)cap, "TX %d", rc);
        return rc;
    }
    rc=core_read_status(reply, err, sizeof(err));
    if(rc!=0) {
        snprintf(out,(size_t)cap, "%s", err);
        AParcel_delete(reply);
        return rc;
    }
    struct core_string_alloc a= {
        out, cap
    };
    out[0]=0;
    rc=AParcel_readString(reply, &a, core_string_allocator);
    AParcel_delete(reply);
...
static int core_call_i32(AIBinder* core, uint32_t code, int32_t* value) {
    ...
    rc=core_read_status(reply, err, sizeof(err));
    if(rc==0)rc=AParcel_readInt32(reply, value);
    AParcel_delete(reply);
    return rc;
}
static int core_call_i64(AIBinder* core, uint32_t code, int64_t* value) {
    ...
    rc=core_read_status(reply, err, sizeof(err));
    if(rc==0)rc=AParcel_readInt64(reply, value);
    AParcel_delete(reply);
    return rc;
}
    static int core_call_policy(AIBinder* core, const char* principal, const char* action, const char* resource, char* out,
int cap) {
    AParcel* in=0;
    AParcel* reply=0;
    char err[128];
    ...
    if(rc!=0) {
        AParcel_delete(in);
        snprintf(out,(size_t)cap, "WRITE %d", rc);
        return rc;
    }
...
static void render_core(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    ...
    if(core_ok) {
        int32_t proto=0;
        int64_t caps=0;
        int rc=core_call_i32(core, 2, &proto);
        if(rc==0) {
            snprintf(v, sizeof(v), "%d", proto);
            kv(b, y, x, "CORE PROTOCOL", v, 1, scale);
        }
        else {
            snprintf(v, sizeof(v), "ERR %d", rc);
            kv(b, y, x, "CORE PROTOCOL", v, -1, scale);
        }
        rc=core_call_i64(core, 3, &caps);
        if(rc==0) {
            snprintf(v, sizeof(v), "0x%llx",(unsigned long long)caps);
            kv(b, y, x, "CORE CAPS", v, 1, scale);
        }
        else {
            snprintf(v, sizeof(v), "ERR %d", rc);
            kv(b, y, x, "CORE CAPS", v, -1, scale);
        }
        rc=core_call_string(core, 4, v, sizeof(v));
        kv(b, y, x, "CORE CALLER", v, rc==0?1:-1, scale);
        rc=core_call_string(core, 5, v, sizeof(v));
        kv(b, y, x, "CORE RUNTIME", v, rc==0?1:-1, scale);
        rc=core_call_string(core, 6, v, sizeof(v));
        kv(b, y, x, "CORE KERNEL", v, rc==0?1:-1, scale);
        rc=core_call_string(core, 7, v, sizeof(v));
        kv(b, y, x, "CORE PROCESS", v, rc==0?1:-1, scale);
        rc=core_call_string(core, 8, v, sizeof(v));
        kv(b, y, x, "CORE NETWORK", v, rc==0?1:-1, scale);
        rc=core_call_string(core, 9, v, sizeof(v));
        kv(b, y, x, "CORE SECURITY", v, rc==0?1:-1, scale);
        rc=core_call_policy(core, "app", "read", "device.status", v, sizeof(v));
        kv(b, y, x, "CORE POLICY", v, rc==0?1:-1, scale);
        if(proto>=2) {
            rc=core_call_string(core, 11, v, sizeof(v));
            kv(b, y, x, "CORE FILESYSTEM", v, rc==0?1:-1, scale);
            rc=core_call_string(core, 12, v, sizeof(v));
            kv(b, y, x, "CORE SYSCTL", v, rc==0?1:-1, scale);
            rc=core_call_string(core, 13, v, sizeof(v));
            kv(b, y, x, "CORE BINDER", v, rc==0?1:-1, scale);
            rc=core_call_string(core, 14, v, sizeof(v));
            kv(b, y, x, "CORE MATRIX", v, rc==0?1:-1, scale);
        }
    }
    if(core)AIBinder_decStrong(core);
    int hello=bridge_request("HELLO", v, sizeof(v));
    if(hello==0) kv(b, y, x, "ADB BRIDGE", v, 1, scale);
    else {
        snprintf(v, sizeof(v), "OFFLINE ERRNO %d", hello);
        kv(b, y, x, "ADB BRIDGE", v, 0, scale);
    }
    if(hello==0 && strstr(v, "auth=required") && g_bridge_token[0]==0) {
        ...
        snprintf(tmp, sizeof(tmp), "AUTHENTICATED SESSION / TOKEN %d CHARS",(int)strlen(g_bridge_token));
        kv(b, y, x, "BRIDGE AUTH", tmp, 1, scale);
    }
    int bridge_ok=0;
    status[0]=0;
    services[0]=0;
    if(hello==0) {
        ...
        if(br==0) {
            bridge_ok=1;
            bridge_save_authenticated_token();
            char ctx[192], sec[32], enf[64];
            bridge_field(status, "context", ctx, sizeof(ctx));
            bridge_field(status, "seccomp", sec, sizeof(sec));
            bridge_field(status, "selinux_enforce", enf, sizeof(enf));
            snprintf(v, sizeof(v), "%s / SECCOMP %s / SELINUX %s", ctx, sec, enf);
            kv(b, y, x, "SHELL CORE", v, 1, scale);
        }
        else {
            snprintf(v, sizeof(v), br==401?"AUTH TOKEN MISSING":"QUERY ERRNO %d", br);
            kv(b, y, x, "SHELL CORE", v, 0, scale);
        }
        br=bridge_request("SERVICES", services, sizeof(services));
        if(br==0) {
            char count[32], tested[32];
            bridge_field(services, "count", count, sizeof(count));
            bridge_field(services, "tested", tested, sizeof(tested));
            snprintf(v, sizeof(v), "%s/%s KNOWN SERVICES VISIBLE", count, tested);
            kv(b, y, x, "SHELL BINDER", v, 1, scale);
        }
    }
    if(bridge_ok) {
        char shellsec[32]="?", enf[64]="?", pnet[32]="?", ctx[192]="?";
        bridge_field(status, "seccomp", shellsec, sizeof(shellsec));
        bridge_field(status, "selinux_enforce", enf, sizeof(enf));
        bridge_field(status, "proc_net", pnet, sizeof(pnet));
        bridge_field(status, "context", ctx, sizeof(ctx));
        if(extract_status("Seccomp:", appsec, sizeof(appsec))<0)snprintf(appsec, sizeof(appsec), "?");
        snprintf(v, sizeof(v), "SECCOMP %s -> %s", appsec, shellsec);
        kv(b, y, x, "APP -> SHELL", v, mini_streq(shellsec, "0")?1:0, scale);
        snprintf(v, sizeof(v), "PROC_NET DENIED -> %s / SELINUX READ -> %s", mini_streq(pnet, "1")?"VISIBLE":"DENIED", enf);
        kv(b, y, x, "BOUNDARY", v, 1, scale);
    ...
    snprintf(v, sizeof(v), "127.0.0.1:%d", ZORIN_BRIDGE_PORT);
    kv(b, y, x, "BRIDGE ENDPOINT", v, bridge_ok?1:0, scale);
...
static const char* g_known_services[]= {
    "activity", "package", "window", "power", "display", "SurfaceFlinger", "sensorservice", "connectivity", "netd",
    "vold",
    "media.audio_flinger", "permission", "mount", "keystore2", "stats", "incident", "gatekeeper", "gpu", "input", "batterystats",
    "deviceidle", "thermalservice", "storaged", "usagestats", "jobscheduler", "appops", "role", "shortcut", "notification",
    "location",
...
static int app_service_map(char* list, int cap, int* ping_ok) {
    int pos=0, found=0, pok=0;
    if(list&&cap>0)list[0]=0;
    for(int i=0; i<KNOWN_SERVICE_COUNT; ++i) {
        AIBinder* svc=AServiceManager_checkService(g_known_services[i]);
        if(!svc)continue;
        int pr=AIBinder_ping(svc);
        AIBinder_decStrong(svc);
        if(pr!=0)continue;
        ++found;
        ++pok;
        if(list&&cap>1) {
            if(pos&&pos<cap-1)list[pos++]=',';
            const char*n=g_known_services[i];
            while(*n&&pos<cap-1)list[pos++]=*n++;
            list[pos]=0;
        }
    }
    if(ping_ok)*ping_ok=pok;
    return found;
}
static int csv_has_token(const char* csv, const char* token) {
    ...
    while(*p) {
        ...
        if(n==tl) {
            int same=1;
            for(int i=0; i<n; ++i)if(s[i]!=token[i]) {
                same=0;
                break;
            }
            if(same)return 1;
...
static void render_binder(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    ...
    for(int i=0; i<3; ++i) {
        ...
        if(fd>=0) {
            snprintf(v, sizeof(v), "OPEN OK FD %d", fd);
            kv(b, y, x, names[i], v, 1, scale);
            close(fd);
        }
        else {
            snprintf(v, sizeof(v), "OPEN FAIL ERRNO %d", last_errno());
            kv(b, y, x, names[i], v, -1, scale);
        }
    }
    int fd=open("/dev/binder", O_RDWR|O_CLOEXEC);
    if(fd>=0) {
        struct binder_version bv;
        bv.protocol_version=0;
        int rc=ioctl(fd, BINDER_VERSION_IOCTL, &bv);
        if(rc==0) {
            snprintf(v, sizeof(v), "PROTOCOL %d", bv.protocol_version);
            kv(b, y, x, "VERSION IOCTL", v, 1, scale);
        }
        else {
            snprintf(v, sizeof(v), "FAIL ERRNO %d", last_errno());
            kv(b, y, x, "VERSION IOCTL", v, -1, scale);
        }
        close(fd);
    }
    else kv(b, y, x, "VERSION IOCTL", "SKIPPED", -1, scale);
    // Прямые запросы к публичному ServiceManager из libbinder_ndk. На экране оставляем компактные классические 12;
    // COPY ALL пишет ту же карту из 36 имён, что shell bridge, чтобы privilege diff был точным.
    ...
    for(int i=0; i<KNOWN_SERVICE_COUNT; ++i) {
        ...
        if(svc) {
            ...
            if(pr==0) ++ping_ok;
            if(!b || i<12) {
                snprintf(v, sizeof(v), "FOUND PING %d %s/%s", pr, AIBinder_isRemote(svc)?"REMOTE":"LOCAL", AIBinder_isAlive(svc)?"ALIVE":"DEAD");
                kv(b, y, x, g_known_services[i], v, pr==0?1:0, scale);
            }
            AIBinder_decStrong(svc);
        }
        else if(!b || i<12) kv(b, y, x, g_known_services[i], "NOT VISIBLE", 0, scale);
    }
    snprintf(v, sizeof(v), "%d/%d FOUND, %d PING OK", found, KNOWN_SERVICE_COUNT, ping_ok);
    kv(b, y, x, "SM SUMMARY", v, found>0?1:0, scale);
...
static void render_network(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    char v[256];
    int n, e=0;
    n=count_file_lines_ex("/proc/net/dev", &e);
    if(n>=2)snprintf(v, sizeof(v), "%d", n-2);
    else snprintf(v, sizeof(v), "DENIED ERRNO %d", e);
    kv(b, y, x, "INTERFACES", v, n>=2?1:0, scale);
    n=count_file_lines_ex("/proc/net/route", &e);
    if(n>=1)snprintf(v, sizeof(v), "%d", n-1);
    else snprintf(v, sizeof(v), "DENIED ERRNO %d", e);
    kv(b, y, x, "IPV4 ROUTES", v, n>=1?1:0, scale);
    n=count_file_lines_ex("/proc/net/tcp", &e);
    if(n>=1)snprintf(v, sizeof(v), "%d", n-1);
    else snprintf(v, sizeof(v), "DENIED ERRNO %d", e);
    kv(b, y, x, "TCP ENTRIES", v, n>=1?1:0, scale);
    n=count_file_lines_ex("/proc/net/udp", &e);
    if(n>=1)snprintf(v, sizeof(v), "%d", n-1);
    else snprintf(v, sizeof(v), "DENIED ERRNO %d", e);
    kv(b, y, x, "UDP ENTRIES", v, n>=1?1:0, scale);
    int fd=socket(AF_INET, SOCK_STREAM, 0);
    if(fd>=0) {
        snprintf(v, sizeof(v), "CREATE OK FD %d", fd);
        kv(b, y, x, "INET SOCKET", v, 1, scale);
        close(fd);
    }
    else {
        snprintf(v, sizeof(v), "FAIL ERRNO %d", last_errno());
    ...
    if(fd>=0) {
        snprintf(v, sizeof(v), "CREATE OK FD %d", fd);
        kv(b, y, x, "UNIX SOCKET", v, 1, scale);
        close(fd);
    }
    else {
        snprintf(v, sizeof(v), "FAIL ERRNO %d", last_errno());
    ...
    if(fd>=0) {
        snprintf(v, sizeof(v), "OPEN OK FD %d", fd);
        kv(b, y, x, "TUN DEVICE", v, 1, scale);
        close(fd);
    }
    else {
        snprintf(v, sizeof(v), "NO ACCESS ERRNO %d", last_errno());
        kv(b, y, x, "TUN DEVICE", v, 0, scale);
    }
    if(prop_get("net.dns1", v, sizeof(v))) kv(b, y, x, "DNS PROP", v, 1, scale);
    else kv(b, y, x, "DNS PROP", "NOT EXPOSED", 0, scale);
    section_note(b, y, x, "INTERNET IS A NORMAL (NON-DANGEROUS) PERMISSION.", scale);
}
static int su_visible(void) {
    const char* p[]= {
        "/system/bin/su", "/system/xbin/su", "/sbin/su", "/su/bin/su", "/debug_ramdisk/su"
    };
    for(int i=0; i<5; ++i) if(access(p[i], F_OK)==0) return 1;
    return 0;
}
static void render_security(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    char v[256], se[64];
    const char* sm=selinux_mode(se, sizeof(se));
    kv(b, y, x, "SELINUX", sm, mini_streq(sm, "ENFORCING")?1:0, scale);
    if(read_small("/proc/self/attr/current", v, sizeof(v))>0) {
        trim_line(v);
        kv(b, y, x, "SECONTEXT", v, 1, scale);
    }
    kv(b, y, x, "SU PATH", su_visible()?"VISIBLE":"NOT VISIBLE", su_visible()?0:1, scale);
    snprintf(v, sizeof(v), "UID %u", getuid());
    kv(b, y, x, "ROOT UID", v, getuid()==0?0:1, scale);
        if(prop_get("ro.boot.verifiedbootstate", v, sizeof(v))) kv(b, y, x, "VERIFIED BOOT", v, mini_streq(v, "green")?1:0,
    scale);
    else kv(b, y, x, "VERIFIED BOOT", "NOT EXPOSED", 0, scale);
        if(prop_get("ro.boot.vbmeta.device_state", v, sizeof(v))) kv(b, y, x, "VBMETA STATE", v, mini_streq(v, "locked")?1:0,
    scale);
...
static void render_native(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    char v[256];
    int ps=getpagesize();
    snprintf(v, sizeof(v), "%d BYTES", ps);
    kv(b, y, x, "PAGE SIZE", v, 1, scale);
    void* p=mmap(0,(size_t)ps, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    if(p!=MAP_FAILED) {
        ((volatile char*)p)[0]=0x5a;
        kv(b, y, x, "MMAP RW", "OK", 1, scale);
        int rc=mprotect(p,(size_t)ps, PROT_READ);
        snprintf(v, sizeof(v), "%s", rc==0?"OK":"FAIL");
        kv(b, y, x, "MPROTECT R", v, rc==0?1:-1, scale);
        munmap(p,(size_t)ps);
    }
    else {
        snprintf(v, sizeof(v), "FAIL ERRNO %d", last_errno());
        kv(b, y, x, "MMAP RW", v, -1, scale);
    }
    int fd=epoll_create1(O_CLOEXEC);
    if(fd>=0) {
        snprintf(v, sizeof(v), "FD %d", fd);
        kv(b, y, x, "EPOLL", v, 1, scale);
        close(fd);
    }
    else {
        snprintf(v, sizeof(v), "ERRNO %d", last_errno());
        kv(b, y, x, "EPOLL", v, -1, scale);
    }
    fd=eventfd(0, O_CLOEXEC);
    if(fd>=0) {
        snprintf(v, sizeof(v), "FD %d", fd);
        kv(b, y, x, "EVENTFD", v, 1, scale);
        close(fd);
    }
    else {
        snprintf(v, sizeof(v), "ERRNO %d", last_errno());
    ...
    int rc=pipe2(pipefd, O_CLOEXEC);
    if(rc==0) {
        snprintf(v, sizeof(v), "FDS %d/%d", pipefd[0], pipefd[1]);
        kv(b, y, x, "PIPE2", v, 1, scale);
        close(pipefd[0]);
        close(pipefd[1]);
    }
    else {
    ...
    int word=0;
    long fr=syscall(FUTEX_NR, &word, FUTEX_WAKE, 1, 0, 0, 0);
    snprintf(v, sizeof(v), "RC %ld", fr);
    kv(b, y, x, "FUTEX WAKE", v, fr>=0?1:-1, scale);
    char sc[32];
    ...
    if(clock_gettime(CLOCK_MONOTONIC, &ts)==0) {
        snprintf(v, sizeof(v), "%ld.%09ld", ts.tv_sec, ts.tv_nsec);
        kv(b, y, x, "MONOTONIC", v, 1, scale);
    }
    section_note(b, y, x, "SAFE PROBES RUN IN-PROCESS; KILLING SYSCALLS ARE NOT RETRIED.", scale);
}
struct child_probe_result {
    long rc;
    int err;
};
    static void run_child_syscall(long nr, long a0, long a1, long a2, long a3, long a4, long a5, char* out, int cap,
int* state) {
    int pfd[2]= {
        -1, -1
    };
    ...
    if(n==(ssize_t)sizeof(r)) {
        if(r.rc>=0) {
            snprintf(out,(size_t)cap, "ALLOWED RC %ld", r.rc);
            *state=1;
        }
        else {
            snprintf(out,(size_t)cap, "ERRNO %d", r.err);
            *state=(r.err==EPERM||r.err==EACCES||r.err==ENOSYS)?0:1;
        }
    }
    else {
        snprintf(out,(size_t)cap, "NO RESULT");
        *state=0;
    }
}
static void render_sandbox(ANativeWindow_Buffer* b, int* y, int x, int scale) {
    char v[128];
    int st=0;
    run_child_syscall(IO_URING_SETUP_NR, 0, 0, 0, 0, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "IO_URING", v, st, scale);
    run_child_syscall(BPF_NR, -1, 0, 0, 0, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "BPF", v, st, scale);
    run_child_syscall(PERF_EVENT_OPEN_NR, 0, 0, -1, -1, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "PERF EVENT", v, st, scale);
    run_child_syscall(USERFAULTFD_NR, 0, 0, 0, 0, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "USERFAULTFD", v, st, scale);
    run_child_syscall(CLONE3_NR, 0, 0, 0, 0, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "CLONE3", v, st, scale);
    run_child_syscall(MOUNT_NR, 0, 0, 0, 0, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "MOUNT", v, st, scale);
    run_child_syscall(SETNS_NR, -1, 0, 0, 0, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "SETNS", v, st, scale);
    run_child_syscall(PTRACE_NR, -1, 0, 0, 0, 0, 0, v, sizeof(v), &st);
    kv(b, y, x, "PTRACE", v, st, scale);
...
static void report_bridge_detail(const char* title, const char* command) {
    char b[8192];
    int rc=bridge_request(command, b, sizeof(b));
    report_append("\n[SHELL CORE / ");
    report_append(title);
    report_append("]\n");
    if(rc==0) {
...
static void build_full_report(void) {
    ...
    report_append("\n[SUMMARY]\n");
    {
        char v[128], ssec[32]="offline", senf[32]="offline", bs[2048], ss[4096], shell_count[32]="offline", shell_found[3072]="",
        app_found[3072]="";
        if(extract_status("Seccomp:", v, sizeof(v))>=0) {
            report_append("app.seccomp=");
            report_append(v);
            report_append("\n");
        }
        if(bridge_request("STATUS", bs, sizeof(bs))==0) {
            bridge_field(bs, "seccomp", ssec, sizeof(ssec));
            bridge_field(bs, "selinux_enforce", senf, sizeof(senf));
        }
        int app_ping=0, app_count=app_service_map(app_found, sizeof(app_found), &app_ping);
        if(bridge_request("SERVICES", ss, sizeof(ss))==0) {
            bridge_field(ss, "count", shell_count, sizeof(shell_count));
            bridge_field(ss, "found", shell_found, sizeof(shell_found));
        }
        char nbuf[64];
        snprintf(nbuf, sizeof(nbuf), "%d", app_count);
        report_append("binder.app_visible=");
        report_append(nbuf);
        report_append("/36\n");
        report_append("binder.shell_visible=");
        report_append(shell_count);
        report_append("/36\n");
        report_append("binder.shell_only=");
        int any=0;
        for(int i=0; i<KNOWN_SERVICE_COUNT; ++i) {
            if(csv_has_token(shell_found, g_known_services[i])&&!csv_has_token(app_found, g_known_services[i])) {
                if(any++)report_append(",");
                report_append(g_known_services[i]);
            }
        }
        if(!any)report_append("none");
        report_append("\n");
        report_append("shell.seccomp=");
        report_append(ssec);
        report_append("\n");
        report_append("shell.selinux_enforce=");
        report_append(senf);
        report_append("\n");
...
static const char* product_pages[4]= {
    "HOME", "REQUESTS", "DEVICES", "SETTINGS"
};
    static void product_button(ANativeWindow_Buffer* b, int x, int y, int w, int h, const char* label, int rect[4],
int scale, int danger) {
        uint32_t bg=danger?rgb(48, 24, 30):rgb(34, 36, 44), edge=danger?rgb(238, 72, 88):rgb(64, 67, 78), fg=danger?rgb(255,
    118, 132):rgb(244, 245, 248);
    fill_rect(b, x, y, w, h, bg);
    outline_rect(b, x, y, w, h, 1, edge);
    int tw=text_width(label, scale);
    draw_text(b, x+(w-tw)/2, y+(h-7*scale)/2, label, scale, fg);
    rect[0]=x;
    rect[1]=y;
    rect[2]=w;
    rect[3]=h;
}
    static void product_status_row(ANativeWindow_Buffer* b, int x, int y, int w, const char* label, const char* value,
int state, int scale) {
    uint32_t dim=rgb(146, 149, 160), fg=rgb(239, 240, 244), red=rgb(238, 72, 88), amber=rgb(214, 162, 79);
    draw_text(b, x, y, label, scale, dim);
    int tw=text_width(value, scale);
    draw_text(b, x+w-tw, y, value, scale, state>0?red:(state==0?amber:fg));
}
static void product_ellipsis(const char* in, char* out, int cap, int maxchars) {
    int i=0;
    if(!in) {
        out[0]=0;
        return;
    }
    while(in[i]&&i<maxchars&&i<cap-1) {
        out[i]=in[i];
        ++i;
    }
    if(in[i]&&i+3<cap) {
...
static void product_update_insets(void) {
    g_safe_left=g_safe_top=g_safe_right=g_safe_bottom=0;
    if(!g_activity||!g_activity->clazz)return;
    JNIEnv* env=0;
    int attached=0;
    if(trust_attach(&env, &attached)!=0||!env)return;
    jclass ac=(*env)->GetObjectClass(env, g_activity->clazz);
    jmethodID gw=ac?(*env)->GetMethodID(env, ac, "getWindow", "()Landroid/view/Window;"):0;
    jobject win=gw?(*env)->CallObjectMethod(env, g_activity->clazz, gw):0;
    jclass wc=win?(*env)->GetObjectClass(env, win):0;
    jmethodID gd=wc?(*env)->GetMethodID(env, wc, "getDecorView", "()Landroid/view/View;"):0;
    jobject decor=gd?(*env)->CallObjectMethod(env, win, gd):0;
    jclass dc=decor?(*env)->GetObjectClass(env, decor):0;
    jmethodID gri=dc?(*env)->GetMethodID(env, dc, "getRootWindowInsets", "()Landroid/view/WindowInsets;"):0;
    jobject wi=gri?(*env)->CallObjectMethod(env, decor, gri):0;
    jclass tc=(*env)->FindClass(env, "android/view/WindowInsets$Type");
    jmethodID sb=tc?(*env)->GetStaticMethodID(env, tc, "systemBars", "()I"):0;
    jint types=sb?(*env)->CallStaticIntMethod(env, tc, sb):0;
    jclass wic=wi?(*env)->GetObjectClass(env, wi):0;
    jmethodID gi=wic?(*env)->GetMethodID(env, wic, "getInsets", "(I)Landroid/graphics/Insets;"):0;
    jobject ins=(gi&&wi)?(*env)->CallObjectMethod(env, wi, gi, types):0;
    jclass ic=ins?(*env)->GetObjectClass(env, ins):0;
        jfieldID fl=ic?(*env)->GetFieldID(env, ic, "left", "I"):0, ft=ic?(*env)->GetFieldID(env, ic, "top", "I"):0, fr=ic?(*env)->GetFieldID(env,
    ic, "right", "I"):0, fb=ic?(*env)->GetFieldID(env, ic, "bottom", "I"):0;
    if(ins&&fl&&ft&&fr&&fb) {
        g_safe_left=(*env)->GetIntField(env, ins, fl);
        g_safe_top=(*env)->GetIntField(env, ins, ft);
        g_safe_right=(*env)->GetIntField(env, ins, fr);
        g_safe_bottom=(*env)->GetIntField(env, ins, fb);
    }
    if((*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        g_safe_left=g_safe_top=g_safe_right=g_safe_bottom=0;
    ...
    if(ic)(*env)->DeleteLocalRef(env, ic);
    if(ins)(*env)->DeleteLocalRef(env, ins);
    if(wic)(*env)->DeleteLocalRef(env, wic);
    if(wi)(*env)->DeleteLocalRef(env, wi);
    if(tc)(*env)->DeleteLocalRef(env, tc);
    if(dc)(*env)->DeleteLocalRef(env, dc);
...
static void render_product_home(ANativeWindow_Buffer* b, int* y, int x, int avail, int scale) {
    (void)trust_ui_sync_from_service();
    int trusted=g_trust_state==3, pending=g_trust_state==1, locked=trust_device_locked();
        uint32_t red=rgb(238, 72, 88), amber=rgb(214, 162, 79), dim=rgb(146, 149, 160), fg=rgb(244, 245, 248), panel=rgb(25,
    27, 34), edge=rgb(46, 49, 59), bg=rgb(15, 16, 21);
    int cx=x+avail/2, r=58*scale;
    if(r>avail/4)r=avail/4;
    int cy=*y+r+6*scale;
    draw_ring(b, cx, cy, r, 8*scale, trusted?(locked?amber:red):(pending?amber:rgb(57, 60, 70)), bg);
    const char* title=g_authority_pending?"APPROVAL NEEDED":(trusted?(locked?"PHONE LOCKED":"YOU'RE PROTECTED"):(pending?"PAIRING":"PHONE OFFLINE"));
    int tw=text_width(title, scale+1);
    draw_text(b, cx-tw/2, cy-5*scale, title, scale+1, g_authority_pending?amber:(trusted?(locked?amber:red):fg));
    *y=cy+r+22*scale;
    fill_rect(b, x, *y, avail, 98*scale, panel);
    outline_rect(b, x, *y, avail, 98*scale, 1, edge);
    int py=*y+15*scale;
        const char* host=(g_trust_host_name[0]&&!mini_streq(g_trust_host_name, "UNKNOWN HOST")&&!mini_streq(g_trust_host_name,
    "<NONE>"))?g_trust_host_name:"Your workstation";
    draw_text(b, x+15, py, host, scale+1, fg);
    py+=23*scale;
    product_status_row(b, x+15, py, avail-30, "Connection", trusted?"Trusted":"Offline", trusted?1:-1, scale);
    py+=18*scale;
        product_status_row(b, x+15, py, avail-30, "Sensitive actions", trusted&&!locked?"Available":"Paused", trusted&&!locked?1:0,
    scale);
    *y+=114*scale;
    if(g_authority_pending) {
        char pbuf[96];
        product_ellipsis(g_authority_prompt, pbuf, sizeof(pbuf), 54);
        draw_text(b, x, *y, "REQUEST", scale, dim);
        *y+=18*scale;
        draw_text(b, x, *y, pbuf, scale+1, fg);
        *y+=26*scale;
            draw_text(b, x, *y, locked?"Unlock your phone, then open Requests to decide.":"Open Requests to authorize or deny this action.",
        scale, locked?amber:dim);
        *y+=22*scale;
    }
    else if(pending) {
        char code[96];
        trust_pair_code(g_trust_host_fp, code, sizeof(code));
        draw_text(b, x, *y, "VERIFY ON BOTH DEVICES", scale, dim);
        *y+=18*scale;
        int cw=text_width(code, scale+1);
        draw_text(b, cx-cw/2, *y, code, scale+1, fg);
        *y+=28*scale;
        product_button(b, x, *y, avail, 38*scale, "TRUST THIS WORKSTATION", g_trust_approve_rect, scale, 0);
        *y+=50*scale;
    }
    else if(trusted) {
            draw_text(b, x, *y, locked?"Trust stays active. Unlock only when you need to approve something.":"Zorin is ready. You do not need to keep this app open.",
        scale, dim);
        *y+=24*scale;
    }
}
static void render_product_requests(ANativeWindow_Buffer* b, int* y, int x, int avail, int scale) {
    (void)trust_ui_sync_from_service();
        uint32_t fg=rgb(244, 245, 248), dim=rgb(146, 149, 160), panel=rgb(25, 27, 34), edge=rgb(46, 49, 59), amber=rgb(214,
    162, 79);
    draw_text(b, x, *y, "REQUESTS", scale, dim);
    *y+=22*scale;
    if(!g_authority_pending) {
        fill_rect(b, x, *y, avail, 130*scale, panel);
        outline_rect(b, x, *y, avail, 130*scale, 1, edge);
        draw_text(b, x+16, *y+18*scale, "Nothing needs your attention", scale+1, fg);
        draw_text(b, x+16, *y+48*scale, "Sensitive actions from apps, websites and Zorin Ops", scale, dim);
        draw_text(b, x+16, *y+66*scale, "will appear here only when a decision is required.", scale, dim);
        return;
    }
    int locked=trust_device_locked();
    fill_rect(b, x, *y, avail, 188*scale, panel);
    outline_rect(b, x, *y, avail, 188*scale, 1, locked?amber:edge);
    int py=*y+16*scale;
    draw_text(b, x+16, py, locked?"UNLOCK TO AUTHORIZE":"ACTION REQUIRES APPROVAL", scale, locked?amber:dim);
    py+=24*scale;
    char pbuf[110], abuf[90], rbuf[110];
    product_ellipsis(g_authority_prompt, pbuf, sizeof(pbuf), 60);
    product_ellipsis(g_authority_action, abuf, sizeof(abuf), 46);
    product_ellipsis(g_authority_resource, rbuf, sizeof(rbuf), 60);
    draw_text(b, x+16, py, pbuf, scale+1, fg);
    py+=28*scale;
    draw_text(b, x+16, py, abuf, scale, dim);
    py+=18*scale;
    draw_text(b, x+16, py, rbuf, scale, dim);
    py+=24*scale;
    int gap=8, w=(avail-32-gap)/2;
    product_button(b, x+16, py, w, 38*scale, "DENY", g_authority_deny_rect, scale, 1);
        product_button(b, x+16+w+gap, py, w, 38*scale, locked?"UNLOCK FIRST":"AUTHORIZE", g_authority_allow_rect, scale,
    0);
    *y+=204*scale;
}
static void render_product_devices(ANativeWindow_Buffer* b, int* y, int x, int avail, int scale) {
    ...
    *y+=20*scale;
    fill_rect(b, x, *y, avail, 96*scale, panel);
    outline_rect(b, x, *y, avail, 96*scale, 1, edge);
    int py=*y+16*scale;
...
static void render_product_settings(ANativeWindow_Buffer* b, int* y, int x, int avail, int scale) {
        uint32_t fg=rgb(244, 245, 248), dim=rgb(146, 149, 160), panel=rgb(25, 27, 34), edge=rgb(46, 49, 59), red=rgb(238,
    72, 88);
    int overlay=trust_overlay_allowed();
    draw_text(b, x, *y, "GENERAL", scale, dim);
    *y+=20*scale;
    fill_rect(b, x, *y, avail, 86*scale, panel);
    outline_rect(b, x, *y, avail, 86*scale, 1, edge);
    draw_text(b, x+16, *y+16*scale, "Connection pulse", scale+1, fg);
    draw_text(b, x+16, *y+40*scale, overlay?"Enabled":"Permission required", scale, overlay?red:dim);
    *y+=100*scale;
    product_button(b, x, *y, avail, 38*scale, overlay?"TEST PULSE":"ENABLE PULSE", g_trust_visual_rect, scale, 0);
    *y+=56*scale;
    ...
    *y+=20*scale;
    ...
    g_product_title_rect[3]=28*scale;
    *y+=25*scale;
    draw_text(b, x, *y, "Developer tools are hidden. Tap the version 7 times to reveal them.", scale, dim);
    *y+=20*scale;
    if(g_developer_mode)draw_text(b, x, *y, "Developer Mode enabled", scale, red);
}
static void render_product(ANativeWindow_Buffer* b) {
    product_update_insets();
    int scale=b->width>=650?2:1;
        uint32_t bg=rgb(15, 16, 21), fg=rgb(246, 246, 248), dim=rgb(146, 149, 160), red=rgb(238, 72, 88), panel=rgb(21,
    23, 29), edge=rgb(43, 46, 55);
    fill_rect(b, 0, 0, b->width, b->height, bg);
    int margin=b->width/18;
    if(margin<20)margin=20;
    int x=g_safe_left+margin;
    int avail=b->width-g_safe_left-g_safe_right-2*margin;
    if(avail<120)avail=b->width-2*margin;
    int y=g_safe_top+22*scale;
    draw_text(b, x, y, "ZORIN", scale+1, fg);
    draw_text(b, x, y+20*scale, "Trust & Authority", scale, dim);
    y+=52*scale;
    if(g_product_page==0)render_product_home(b, &y, x, avail, scale);
    else if(g_product_page==1)render_product_requests(b, &y, x, avail, scale);
    else if(g_product_page==2)render_product_devices(b, &y, x, avail, scale);
    else render_product_settings(b, &y, x, avail, scale);
    int navh=46*scale, navy=b->height-g_safe_bottom-navh;
    if(navy<g_safe_top+100)navy=b->height-navh;
    fill_rect(b, g_safe_left, navy, b->width-g_safe_left-g_safe_right, navh, panel);
    outline_rect(b, g_safe_left, navy, b->width-g_safe_left-g_safe_right, navh, 1, edge);
    int w=(b->width-g_safe_left-g_safe_right)/4;
    for(int i=0; i<4; ++i) {
        uint32_t c=i==g_product_page?red:dim;
        int tw=text_width(product_pages[i], scale);
        draw_text(b, g_safe_left+i*w+(w-tw)/2, navy+(navh-7*scale)/2, product_pages[i], scale, c);
        g_product_nav[i][0]=g_safe_left+i*w;
        g_product_nav[i][1]=navy;
        g_product_nav[i][2]=w;
        g_product_nav[i][3]=navh;
    }
}
static const char* tab_names[9]= {
    "SYSTEM", "PROCESS", "CENTER", "CORE", "BINDER", "NETWORK", "SECURITY", "NATIVE", "SANDBOX"
};
static void draw_button(ANativeWindow_Buffer* b, int idx, int x, int y, int w, int h, int selected, int scale) {
        uint32_t accent=rgb(238, 72, 88), panel=rgb(18, 28, 39), panel2=rgb(25, 39, 52), fg=rgb(222, 232, 240), dim=rgb(130,
    151, 169);
    fill_rect(b, x, y, w, h, selected?panel2:panel);
    outline_rect(b, x, y, w, h, selected?3:1, selected?accent:rgb(48, 67, 82));
    int tw=text_width(tab_names[idx], scale);
    int tx=x+(w-tw)/2;
...
static void render(void) {
    ...
    if(ANativeWindow_lock(win, &b, 0)!=0) return;
    g_width=b.width;
    g_height=b.height;
    if(!g_developer_mode) {
        render_product(&b);
        ...
        uint32_t bg=rgb(7, 12, 18), panel=rgb(14, 22, 31), panel2=rgb(10, 17, 24), fg=rgb(226, 234, 241), dim=rgb(125, 146,
    164), accent=rgb(238, 72, 88);
    fill_rect(&b, 0, 0, b.width, b.height, bg);
    int margin=b.width/45;
    if(margin<16)margin=16;
    if(margin>32)margin=32;
    int scale=b.width>=650?2:1;
    int title_scale=b.width>=650?3:2;
    int y=26;
    ...
    y += 15*scale;
    fill_rect(&b, margin, y, b.width-2*margin, 2, accent);
    y += 12;
    ...
    for(int row=0; row<3; ++row) {
        for(int col=0; col<3; ++col) {
            int i=row*3+col;
            draw_button(&b, i, margin+col*(bw3+gap), y, bw3, bh, g_selected_tab==i, scale);
        }
        y += bh +(row<2?gap:14);
    }
    int runh=28*scale;
    ...
    fill_rect(&b, margin, y, runw, runh, rgb(17, 35, 34));
    outline_rect(&b, margin, y, runw, runh, 2, accent);
    char rb[64];
    snprintf(rb, sizeof(rb), "RUN / REFRESH  #%u", g_run_counter);
    int rtw=text_width(rb, scale);
    ...
    fill_rect(&b, copyx, y, copyw, runh, rgb(23, 34, 45));
    outline_rect(&b, copyx, y, copyw, runh, 2, accent);
    int ctw=text_width(g_copy_status, scale);
    int ctx=copyx+(copyw-ctw)/2;
    if(ctx<copyx+4)ctx=copyx+4;
    ...
    g_copy_rect[3]=runh;
    y += runh+14;
    fill_rect(&b, margin, y, b.width-2*margin, b.height-y-24, panel2);
    fill_rect(&b, margin, y, 5, b.height-y-24, accent);
    int cx=margin+18, cy=y+16;
    char cap[64];
    snprintf(cap, sizeof(cap), "[%s PROBE]", tab_names[g_selected_tab]);
    draw_text(&b, cx, cy, cap, scale, fg);
    cy += 15*scale;
    ...
    int foot=b.height-38*scale;
        if(foot>cy+10) draw_text(&b, cx, foot, "DEVICE TRUST / OWNER PRESENCE / AUTHORITY / TRANSPORT ARE SEPARATE SECURITY STATES.",
    scale, dim);
    ANativeWindow_unlockAndPost(win);
}
static void handle_touch(int x, int y) {
    (void)trust_ui_sync_from_service();
    if(!g_developer_mode) {
        for(int i=0; i<4; ++i)if(in_rect(x, y, g_product_nav[i])) {
            g_product_page=i;
            render();
            return;
        }
        if(g_product_page==0 && in_rect(x, y, g_trust_approve_rect)) {
            if(g_trust_state==1)(void)trust_ui_send_command("APPROVE", g_trust_host_pub_pending);
            render();
            return;
        }
        if(g_product_page==1 && g_authority_pending && in_rect(x, y, g_authority_deny_rect)) {
            (void)trust_ui_send_command("AUTH_DENY", g_authority_request_id);
            render();
            return;
        }
        if(g_product_page==1 && g_authority_pending && in_rect(x, y, g_authority_allow_rect)) {
            if(!trust_device_locked())(void)trust_ui_send_command("AUTH_APPROVE", g_authority_request_id);
            render();
            return;
        }
        if(g_product_page==2 && in_rect(x, y, g_trust_forget_rect)) {
            (void)trust_ui_send_command("FORGET", "");
            render();
            return;
        }
        if(g_product_page==3 && in_rect(x, y, g_trust_visual_rect)) {
            if(trust_overlay_allowed())(void)trust_start_service_from_context(runtime_context(), 1);
            else(void)trust_request_overlay_permission();
            render();
            return;
        }
        if(g_product_page==3 && in_rect(x, y, g_product_title_rect)) {
            if(++g_product_tap_count>=7) {
                g_developer_mode=1;
                g_selected_tab=2;
                g_product_tap_count=0;
            }
            render();
            return;
        }
    ...
    for(int i=0; i<9; ++i) if(in_rect(x, y, g_tab_rects[i])) {
        g_selected_tab=i;
        ++g_run_counter;
        render();
        return;
    }
    if(g_selected_tab==2 && in_rect(x, y, g_trust_approve_rect)) {
        if(g_trust_state==1) {
                if(trust_ui_send_command("APPROVE", g_trust_host_pub_pending)==0) snprintf(g_trust_status, sizeof(g_trust_status),
            "APPROVAL SENT TO TRUST SERVICE");
            else snprintf(g_trust_status, sizeof(g_trust_status), "APPROVAL IPC FAILED");
        }
        ++g_run_counter;
        render();
        return;
    }
    if(g_selected_tab==2 && in_rect(x, y, g_trust_forget_rect)) {
        (void)trust_ui_send_command("FORGET", "");
        ++g_run_counter;
        render();
        return;
    }
    if(g_selected_tab==2 && in_rect(x, y, g_trust_visual_rect)) {
        if(trust_overlay_allowed())(void)trust_start_service_from_context(runtime_context(), 1);
        else(void)trust_request_overlay_permission();
        ++g_run_counter;
        render();
    ...
    if(in_rect(x, y, g_run_rect)) {
        ++g_run_counter;
        snprintf(g_copy_status, sizeof(g_copy_status), "COPY ALL");
        render();
        return;
    }
...
static void* trust_ui_refresh_thread(void* arg) {
    ...
    while(g_running && g_ui_alive && generation==g_ui_generation) {
        int had_authority_request=g_authority_pending;
        int changed=trust_ui_sync_from_service();
        if(!had_authority_request && g_authority_pending && !g_developer_mode) g_product_page=1;
...
static void on_window_redraw(ANativeActivity* a, ANativeWindow* w) {
    (void)a;
    g_window=w;
    render();
}
...
static void on_focus(ANativeActivity* a, int focus) {
    if(focus) {
        ANativeActivity_setWindowFlags(a, FLAG_FULLSCREEN|FLAG_KEEP_SCREEN_ON|FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS, 0);
        render();
    }
}
...
static void on_input_destroyed(ANativeActivity* a, AInputQueue* q) {
    (void)a;
    if(g_input_queue==q)g_input_queue=0;
}
...
void ANativeActivity_onCreate(ANativeActivity* activity, void* savedState, size_t savedStateSize) {
    ...
    g_running=1;
    g_ui_alive=1;
    unsigned int ui_generation=++g_ui_generation;
    // Долгоживущим worker владеет foreground TrustService. Activity теперь только UI.
    // Если OEM policy неожиданно отклонит запуск сервиса, откатываемся к in-process worker.
    if(trust_start_service_from_context(activity->clazz, 0)!=0) trust_start_worker();
    activity->instance=activity;
    bridge_token_from_intent();
    ...
    activity->callbacks->onDestroy=on_destroy;
        ANativeActivity_setWindowFlags(activity, FLAG_FULLSCREEN|FLAG_KEEP_SCREEN_ON|FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS,
    0);
    pthread_t t;
    if(pthread_create(&t, 0, input_thread,(void*)(unsigned long)ui_generation)==0) pthread_detach(t);
    pthread_t rt;
    if(pthread_create(&rt, 0, trust_ui_refresh_thread,(void*)(unsigned long)ui_generation)==0) pthread_detach(rt);
}
