...
function Get-ZorinApkSigner {
  ...
  if(-not(Test-Path $bt)){throw "Android SDK Build-Tools are not installed under $bt"}
  $items=@(Get-ChildItem $bt -Directory | Sort-Object Name -Descending)
  foreach($d in $items){$p=Join-Path $d.FullName 'apksigner.bat'; if(Test-Path $p){return $p}}
...
function Sign-ZorinRuntime([string]$UnsignedApk,[string]$OutputApk) {
  if(-not(Test-Path $UnsignedApk)){throw "Unsigned APK missing: $UnsignedApk"}
